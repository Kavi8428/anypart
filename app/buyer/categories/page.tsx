...
export default async function CategoryPage({ searchParams }: {
    ...
    const brandId = params.brandId ? parseInt(params.brandId) : undefined;
    const modelName = params.modelName ? decodeURIComponent(params.modelName) : undefined;
    const yearId = params.yearId ? parseInt(params.yearId) : undefined;
    const vModelId = params.vModelId ? parseInt(params.vModelId) : undefined;
    const partId = params.partId ? parseInt(params.partId) : undefined;
    const searchQuery = params.q ? decodeURIComponent(params.q) : undefined;

    // Determine Logic State
    let currentStep = 1;
    let title = "Select Your Vehicle Brand";
    let backLink: string | undefined = "/buyer/categories";

    // Fetch Data Context for Headers/Breadcrumbs
    let brandDetails: { name: string } | null = null;
    let vModelDetails: { v_years: { year: number } | null } | null = null;
    let partDetails: { name: string } | null = null;
    ...
    } else if (currentStep === 3 && brandId && modelName) {
        years = await getYearsByModelName(brandId, modelName);
    } else if (currentStep === 4 && vModelId) {
        parts = await getPartNamesByVModel(vModelId, yearId);
    } else if (currentStep === 5 && vModelId && partId) {
                        ...
                        <div>
                            ...
                            {currentStep > 0 && (
                                <p className="text-gray-500 text-xs sm:text-sm mt-0.5">
                                    {currentStep === 5 ? "Browse available parts below." : `Step ${currentStep} of 4`}
                                </p>
                            )}
                        </div>
                    </div>
