'use server';

import { prisma } from '@/lib/prisma';
import { getSellerSession } from '@/lib/auth';
import { revalidatePath } from 'next/cache';
import { unlink } from 'node:fs/promises';
import { join } from 'node:path';
import { existsSync } from 'node:fs';
import { saveImage } from '@/lib/image-server';
import { normalizePagination, paginatedResponse } from '@/lib/pagination';
import { APP_CONFIG } from '@/lib/config';
import { ProductSchema } from '@/lib/schemas';

export async function getProducts(page: number = 1, limit?: number) {
    const session = await getSellerSession();
    if (!session) {
        throw new Error('Unauthorized');
    }

    const { page: normalizedPage, limit: normalizedLimit, skip } = normalizePagination({
        page,
        limit: limit || APP_CONFIG.PAGINATION.DEFAULT_PAGE_SIZE
    });

    try {
        const [products, total] = await Promise.all([
            prisma.seller_products.findMany({
                where: {
                    seller_id: session.seller_id,
                    deleted_at: null,
                },
                include: {
                    p_name_ref: true,
                    v_model_ref: {
                        include: {
                            v_brands: true,
                        },
                    },
                    v_year_ref: true,
                    condition_ref: true,
                    hash_tag_1_ref: true,
                    hash_tag_2_ref: true,
                    hash_tag_3_ref: true,
                    seller_payments: {
                        include: {
                            status_ref: true,
                            method_ref: true
                        }
                    },
                },
                orderBy: {
                    created_at: 'desc',
                },
                skip,
                take: normalizedLimit,
            }),
            prisma.seller_products.count({
                where: {
                    seller_id: session.seller_id,
                    deleted_at: null,
                },
            }),
        ]);

        return paginatedResponse(products, normalizedPage, normalizedLimit, total);
    } catch (error) {
        console.error('Error fetching products:', error);
        throw new Error('Failed to fetch products');
    }
}

export async function saveProduct(formData: FormData) {
    const session = await getSellerSession();
    if (!session) {
        throw new Error('Unauthorized');
    }

    try {
        const rawData = Object.fromEntries(formData.entries());
        const validation = ProductSchema.safeParse(rawData);

        if (!validation.success) {
            throw new Error(validation.error.issues[0].message);
        }

        const data = validation.data;
        const { p_name, v_model, v_year, price, condition, description, hash_tag_1, hash_tag_2, hash_tag_3, is_featured } = data;

        if (v_year) {
            const yearExists = await prisma.v_years.findUnique({ where: { id: v_year } });
            if (!yearExists) {
                throw new Error('Invalid vehicle year selected');
            }
        }

        // Handle File uploads
        const img1 = formData.get('image_url_1') as File | null;
        const img2 = formData.get('image_url_2') as File | null;
        const img3 = formData.get('image_url_3') as File | null;

        const image_url_1 = await saveImage(img1) || "placeholder.jpg";
        const image_url_2 = await saveImage(img2);
        const image_url_3 = await saveImage(img3);

        const newFiles = [image_url_1, image_url_2, image_url_3].filter(f => f && f !== "placeholder.jpg") as string[];

        const order_id = formData.get('order_id') as string;

        try {
            await prisma.$transaction(async (tx) => {
                let payment_id: number | null = null;
                if (order_id) {
                    const payment = await tx.seller_payments.findFirst({
                        where: { order_id }
                    });
                    if (payment) payment_id = payment.id;
                }

                await tx.seller_products.create({
                    data: {
                        seller_id: session.seller_id,
                        p_name,
                        v_model,
                        v_year,
                        price,
                        condition,
                        description: description || "",
                        image_url_1,
                        image_url_2,
                        image_url_3,
                        hash_tag_1,
                        hash_tag_2,
                        hash_tag_3,
                        is_featured,
                        payment_id,
                    },
                });
            });
        } catch (txError) {
            // Cleanup files on failure
            for (const f of newFiles) {
                const p = join(process.cwd(), 'public', 'products', f);
                if (existsSync(p)) await unlink(p).catch(() => { });
            }
            throw txError;
        }

        revalidatePath('/seller/products');
        return { success: true };
    } catch (error) {
        console.error('Error saving product:', error);
        const message = error instanceof Error ? error.message : 'Failed to save product. Please check your inputs.';
        throw new Error(message);
    }
}

export async function updateProduct(id: number, formData: FormData) {
    const session = await getSellerSession();
    if (!session) {
        throw new Error('Unauthorized');
    }

    const parseOptionalInt = (val: string | null) => {
        if (!val || val === "" || val === "undefined" || val === "none") return null;
        const parsed = parseInt(val);
        return isNaN(parsed) ? null : parsed;
    };

    try {
        const p_name = parseInt(formData.get('p_name') as string);
        const v_model = parseInt(formData.get('v_model') as string);
        const v_year = parseOptionalInt(formData.get('v_year') as string);
        const price = parseFloat(formData.get('price') as string);
        const condition = parseOptionalInt(formData.get('condition') as string);
        const description = formData.get('description') as string;
        const hash_tag_1 = parseInt(formData.get('hash_tag_1') as string);

        if (isNaN(p_name) || isNaN(v_model) || isNaN(price) || isNaN(hash_tag_1)) {
            throw new Error('Missing or invalid required fields (Part Name, Model, Price, or Tag 1)');
        }

        if (v_year) {
            const yearExists = await prisma.v_years.findUnique({ where: { id: v_year } });
            if (!yearExists) {
                throw new Error('Invalid vehicle year selected');
            }
        }

        // Handle File uploads
        const img1 = formData.get('image_url_1') as File | null;
        const img2 = formData.get('image_url_2') as File | null;
        const img3 = formData.get('image_url_3') as File | null;

        const new_image_url_1 = await saveImage(img1);
        const new_image_url_2 = await saveImage(img2);
        const new_image_url_3 = await saveImage(img3);

        const order_id = formData.get('order_id') as string;

        interface ProductUpdateData {
            p_name: number;
            v_model: number;
            v_year: number | null;
            price: number;
            condition: number | null;
            description: string;
            hash_tag_1: number;
            hash_tag_2: number | null;
            hash_tag_3: number | null;
            is_featured: number;
            image_url_1?: string;
            image_url_2?: string | null;
            image_url_3?: string | null;
            payment_id?: number | null;
        }

        const data: ProductUpdateData = {
            p_name,
            v_model,
            v_year,
            price,
            condition,
            description,
            hash_tag_1,
            hash_tag_2: parseOptionalInt(formData.get('hash_tag_2') as string),
            hash_tag_3: parseOptionalInt(formData.get('hash_tag_3') as string),
            is_featured: formData.get('is_featured') === 'on' ? 1 : (formData.get('is_featured') === '1' ? 1 : 0),
        };

        if (new_image_url_1) data.image_url_1 = new_image_url_1;
        if (new_image_url_2) data.image_url_2 = new_image_url_2;
        if (new_image_url_3) data.image_url_3 = new_image_url_3;

        const newFiles = [new_image_url_1, new_image_url_2, new_image_url_3].filter(f => f) as string[];

        try {
            await prisma.$transaction(async (tx) => {
                if (order_id) {
                    const payment = await tx.seller_payments.findFirst({
                        where: { order_id }
                    });
                    if (payment) data.payment_id = payment.id;
                }

                await tx.seller_products.update({
                    where: {
                        id,
                        seller_id: session.seller_id,
                    },
                    data,
                });
            });
        } catch (txError) {
            // Cleanup NEW files on failure
            for (const f of newFiles) {
                const p = join(process.cwd(), 'public', 'products', f);
                if (existsSync(p)) await unlink(p).catch(() => { });
            }
            throw txError;
        }

        revalidatePath('/seller/products');
        return { success: true };
    } catch (error) {
        console.error('Error updating product:', error);
        throw new Error('Failed to update product. Please check your inputs.');
    }
}

export async function deleteProduct(id: number) {
    const session = await getSellerSession();
    if (!session) {
        throw new Error('Unauthorized');
    }

    try {
        await prisma.seller_products.update({
            where: {
                id,
                seller_id: session.seller_id,
            },
            data: {
                deleted_at: new Date(),
            },
        });
        revalidatePath('/seller/products');
        return { success: true };
    } catch (error) {
        console.error('Error deleting product:', error);
        throw new Error('Failed to delete product');
    }
}
